export default {
    artistFull: "EKV",
  image: "/images/ekv.jpg",
  bio: `
  Katarina II je nastala 1982. godine u Beogradu. 
  Grupu su nakon raspada benda Šarlo Akrobata formirali pevač                  
  Milan Mladenović, gitarista Dragomir „Gagi“ Mihailović, 
  basista Švaba Radomirović i bubnjar Dušan Dejanović. 
  Naziv potiče od imena Mihailovićeve neuzvraćene ljubavi, 
  devojke Katarine. 
  Tokom 1982. bendu se pridružuje klavijaturistkinja Margita 
  „Magi“ Stefanović. 
  Radomirović ubrzo potom prelazi u 
  "Du Du A", a Dušan Dejanović u Disciplinu kičme, 
  te bubnjeve jedno vreme svira Branko „Mango“ Kuštrin. 
  Početkom 1983. na mesto bubnjara dolazi Ivica „Vd“ Vdović, 
  bivši bubnjar Šarla akrobate, a basista postaje Bojan Pečar, 
  čime je formirana konačna ritam sekcija benda. 
  Konačna i poslednja postava grupe Katarina II - Mladenović, 
  Mihailović, Stefanović, 
  Pečar i Vdović - započinje 1983. sa radom na debi albumu

  `,
  discography: [
       "Studijski albumi",
       "Katarina II, 1984.",
       "Ekatarina Velika, 1985.",
       "S' vetrom uz lice, 1986.",
       "Ljubav, 1987.",
       "Samo par godina za nas, 1989.",
       "Dum dum, 1991.",
       "Neko nas posmatra, 1993.",
    "ALBUMI UZIVO:",
       "EKV 19LIVE!86, 1987.",
       "Live '88, 1997.",
       "Kao u snu: EKV - Live '91., 2001.",
    "KOMPILACIJE:",
       "EKV, 1995.",
       "Ovo je zemlja za nas, 2002.",
       "EKV revisited, 2007.",
    "TRIBJUT ALBUMI:",
       "Jako dobar tattoo - Tribute to EKV, 2003.",
       "Kao da je bilo nekad - posvećeno Milanu Mladenoviću, 2003",
    

  ],
  songs: [
    {
      id: "7-dana",
      title: "7 Dana",
      content: `

      Gm                 Eb               Cm      Eb
I sedam dana sam i gore i dole, potrazi me
I sedam dana sam, nadji i stavi, pokazi me

Nek boli dok boli, rukama stegni kozu svoju
Poda mnom zategni strah, noktima pucaj u znoju

Jos ovaj put dok trazi i zove zaklonicu
Jos ovaj put dok zraci i moli poklonicu

Drzi mene, voli mene
Uzmi moj sakom vlazni grli vrat
I sacuvaj vreme i seme za mene
Bio bih tvoj brat, ti si moj brat

D           C
Slomi se, o skloni se
Slomi se, o skloni se

   B     A     Ab
Od mene
   Gm    Fm    Em    Fm
Od mene       od mene

`,
    },
    {
      id: "iznad-grada",

      title: "Iznad grada",
      content: `

      D    Bb     4X


D                  Bb
  Pojeli smo svako zrno,

     D             Bb
Celu noæ strpali u nedra.

D                   Bb
  Popili smo sve do kapi,

     D             Bb
Suve usne duvaju u jedra.


D                C
  Putujemo iznad grada,

        Bm              Bb
Grad se smeje, gleda na nas.

D               C
  Bežimo u novo jutro,

      Bm           Bb
Iznad grada, ispod nas.


D    F      4X


Spojili smo dlan i dlan,
Promenili noæ za dan.
Ne gubimo dah, ne gubimo dah,
Mala deca ne poznaju strah.

Putujemo iznad grada,
Grad se smeje, gleda na nas.
Bežimo u novo jutro,
Iznad grada, ispod nas.

Iznad grada...


Akordi:

D:  x x 0 2 3 2
Bb: x 1 3 3 3 1
C:  x 3 2 0 1 0
Bm: x 2 4 4 3 2
F:  1 3 3 2 1 1

      
`,
    },
{
      id: "anestezija",
      title: "Anestezija",
      content: `

Uvod: Am, Em

Strofa:
Am
Dobro znaš svaku reč, svaki znak
                                      Em
Kako najlakše, najbrže, najslađe reći odlazim
Am
Rastemo, pazimo, slučajno gazimo,
                         Em
Tvoje najblaže usne kažu odlazim


Refren:
     Am    [prelaz]
Anestezija
     Am    [prelaz]
Anestezija


Prelaz:
E|----------------|----------------|----------------|0---------------|
H|8p7-8-------7---|----5-------3---|----7-------5---|0---------------|
G|9p7-9-------7---|----5-------4---|----7-------5---|0---------------|
D|9p7-9-------9---|----7-------5---|----9-------7---|2---------------|
A|----------------|----------------|----------------|2---------------|
E|----------------|----------------|----------------|0---------------|

Smeje se zrcalo mojim očima
Srebrne bube zriču odlazim
Zamisli ti si ja, zamisli ja sam ti
Otvaram oči i kažem odlazim.

      `,
    },

{
      id: "ti-si-sav-moj-bol",
      title: "Ti si sav moj bol",
      content: `

Bsus4        Asus4      E         Em
Razmices zavese, gledas obecani grad.

Bsus4            Asus4         E      Em
Svetla se pale i tinja zelja u tebi.

    Bsus4          Asus4      E    Em
Jos uvek ti djavoli vire iz rukava,

  Bsus4             Asus4      E    Em
I svaki nokat krije otrov ljubavi.

Ti si sav moj bol,
Ti si sav moj bol...

Ispitujes zaborav, obaras alibi
I biras ulogu kao haljinu za vece.
U prolazu me dotakne miris noci, i kaze
Probudi se, pokreni se, probudi se, pokreni se.

Ti si sav moj bol,
Ti si sav moj bol...

Ispitujes zaborav, obaras alibi
I biras ulogu kao haljinu za vece.
U prolazu me dotakne miris noci, i kaze
Probudi se, pokreni se, probudi se, pokreni se.

Ti si sav moj bol,
Ti si sav moj bol...

  `,
    },
    {
      id: "neposmatran",
      title: "Neko nas posmatra",
      content: `
B   A   F#

B      A    F#
   Uno, duo, tre, quatro

B   A   F#   2X

B              A
   Nisam sve izgubio,

F#
   Nešto sam i dobio.

B               A
   Nisam sve prokockao,

F#
   Nešto je i ostalo.

Neko nas posmatra,
Neko nas stalno gleda.
Neko nas posmatra,
Neko nas stalno gleda.

A5   G5   B5
E5
   Neko nas posmatra.

A5   G5   B5
E5
   Neko nas posmatra.


E   C#    4X

Nije sve izgorelo,
Nešto sam ugasio.
Nisam sve izgubio,
Ipak sam se snašao.

Neko nas posmatra,
Neko nas stalno gleda.
Neko nas posmatra,
Neko nas stalno gleda.

Neko nas posmatra.
Neko nas posmatra.

Neko nas posmatra,
Neko nas stalno gleda.
Neko nas posmatra,
Neko nas stalno gleda,
Ko nas stalno gleda...


Akordi/rifovi:

 B: 799877
 A: 577655
F#: 244322
A5: 577xxx
G5: 355xxx
B5: 799xxx
E5: 022xxx
 E: x79997
C#: x46664

      `,
    },
{
      id: "zajedno",
      title: "Zajedno",
      content: `

Strofa:
  E5      D5      A       D5 E5
E-------------------------------
H-----------------5-------------
G--9------7-------6-------7--9--
D--9------7-------7-------7--9--
A--7------5-------7-------5--7--
E-------------------------------

  E5     D5     A      D5 E5  E5   D5   A    D5 E5
  Ivan i Bojan, Mitar, Sa-sa, Orhan
  Nenad i Zoran, Dusan, Vojkan, Ljuba

E-----------------------------------
H-----------------------------------
G--------------------------------9--
D--------------------------------9--
A-2--2------4--4------5--5-------7--
E-0--0------0--0------0--0-------0--          (opet)

  U proslim danima               osvojili smo ponos
  U proslim danima 		 izgubili smo sve

  E5      D5           A5   D5 E5
  Ostajem sam, ostajem sam, 
  U ovom gradu strahom zatrpan
  Ovaj grad je nekad bacao svetla daleko
  Ovaj grad bi mogao bolje

  
  G              Em         G  A
E---------------------------------------
H-----------------------X---------XX-XX-
G-----7----------9-\\---X---------XX-XX-
D-7h9-----9p7----9-\\---X---5--7--XX-XX-
A------------10--7-\\---X---5--7--XX-XX-
E---------------------------3--5--------


  E5     D5           A      D5 E5    E5   D5   A   D5 E5
  Mi smo dozvolili da stvari odu  predaleko
  Ova je borba bila ispod casti, protiv razuma


  U proslim danima osvojili smo ponos
  U proslim danima izgubili smo sve

  E5     D5          A   D5 E5
  Dolazi dan, dolazi dan
  Dan koji cekam ovde zatrpan
  Ovi ljudi su valjda zasluzili bolje
  Mi to mozemo zajedno

  U proslim danima osvojili smo ponos
  U proslim danima izgubili smo sve

  Dolazi dan, dolazi dan
  Dan koji cekam ovde zatrpan
  Ovi ljudi su valjda zasluzili bolje
  Mi to mozemo zajedno

 `,
    },
{
      id: "zemlja",
      title: "Zemlja",
      content: `
    A
E----------------------
B-----2--2-------------
G----2--2--2-----------
D----------------------
A--0-------------------
E----------------------...i samo se ponavlja

A
Ovo je zemlja za nas
ovo je zemlja za sve nase ljude
ovo je kuca za nas
ovo je kuca za svu nasu decu
C             E
Pogledaj me o pogledaj me
          A
ocima deteta

(i tako dalje sve do...)

      A
Cujem vrati se
      G
cujem ostani
      A
cujem vrati se
C        E A
dozvoli mi

Ako primetite neke greske (a ne mozete ih primetiti jer  
ih jednostavno nema) posaljite mi e-mail sa ispravkom. 

Intro riff: transkripcija Ivan Vasic (ivasic@cent.co.yu)

E-----------------|
H-----------------|
G------6br---6----| ponavljati kroz celu pesmu
D----7-----7---5--| 
A--0--------------|
E-----------------|


      `,
    },

  ],
}
